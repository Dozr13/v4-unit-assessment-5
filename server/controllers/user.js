...
module.exports = {
  register: async (req, res) => {
    ...
    if(existingUser){
      return res.status(409).send('Username is already taken. Please try again with a new username!')
    }
    ...
    return res.status(201).send(req.session.username)
  },
  ...
  login: async (req, res) => {
    ...
    if(!user){
      return res.status(401).send('No user found, register as a new user')
    }
